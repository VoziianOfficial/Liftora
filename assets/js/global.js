"use strict";
    ...
    (function () {
        ...
        function applyGlobalBusinessConfig() {
            ...
            const brandName = getConfig("brand.name", "Liftora");
            const tagline = getConfig("brand.tagline", "");
            const shortTagline = getConfig("brand.shortTagline", "");
            const phone = getConfig("contact.phone", "");
            const phoneText = getConfig("contact.phoneText", "");
            const email = getConfig("contact.email", "");
            const address = getConfig("contact.address", "");
            const companyId = getConfig("contact.companyId", "");

            const phoneHref = toTel(phone);
            const emailHref = toMailto(email);

            const replacements = [
                ...business.oldBrandNames.map((from) => [from, brandName]),
                ...business.oldPhones.map((from) => [from, phone]),
                ...business.oldEmails.map((from) => [from, email]),
                ...business.oldAddresses.map((from) => [from, address]),
                ...business.oldCompanyIds.map((from) => [from, companyId])
            ].filter((pair) => pair[0] && pair[1]);

            replaceDocumentTitle(replacements);
            replaceMetaContent(replacements);
            ...
            if (tagline) {
                document.documentElement.style.setProperty("--site-tagline", `"${tagline}"`);
            }

            if (shortTagline) {
                document.documentElement.style.setProperty("--site-short-tagline", `"${shortTagline}"`);
            }

            if (phoneText) {
                document.documentElement.style.setProperty("--site-phone-text", `"${phoneText}"`);
            }
        }

        function replaceDocumentTitle(replacements) {
            document.title = applyTextReplacements(document.title, replacements);
        }

        function replaceMetaContent(replacements) {
            document.querySelectorAll("meta[content]").forEach((meta) => {
                const content = meta.getAttribute("content");
                if (!content) return;

                meta.setAttribute("content", applyTextReplacements(content, replacements));
            });
        }
        ...
        function replaceCommonAttributes(replacements) {
            ...
            document.querySelectorAll("*").forEach((element) => {
                ...
                attributeNames.forEach((attr) => {
                    ...
                    element.setAttribute(attr, applyTextReplacements(currentValue, replacements));
                });
            });
        }
        ...
        function replaceTextNodes(root, replacements) {
            ...
            nodes.forEach((node) => {
                node.nodeValue = applyTextReplacements(node.nodeValue, replacements);
            });
        }
        ...
        function updatePhoneLinks(phoneHref, phoneText) {
            if (!phoneHref || !phoneText) return;

            document.querySelectorAll('a[href^="tel:"]').forEach((link) => {
                link.setAttribute("href", phoneHref);

                if (isHeaderPhoneIconButton(link)) {
                    ensurePhoneIconOnly(link);
                    return;
                }

                const textTarget = link.querySelector("[data-config='contact.phone']") || getTextTargetInsideLink(link);

                if (textTarget && shouldReplaceContactText(textTarget.textContent)) {
                    textTarget.textContent = phoneText;
                }
            });

            document.querySelectorAll("[data-phone-link], [data-config-href='contact.phone']").forEach((element) => {
                element.setAttribute("href", phoneHref);

                if (isHeaderPhoneIconButton(element)) {
                    ensurePhoneIconOnly(element);
                    return;
                }

                if (!element.textContent.trim() || shouldReplaceContactText(element.textContent)) {
                    const textTarget = getTextTargetInsideLink(element);
                    if (textTarget) {
                        textTarget.textContent = phoneText;
                    }
                }
        ...
        function updateEmailLinks(emailHref, emailText) {
            if (!emailHref || !emailText) return;

            document.querySelectorAll('a[href^="mailto:"]').forEach((link) => {
                link.setAttribute("href", emailHref);

                const textTarget = link.querySelector("[data-config='contact.email']") || getTextTargetInsideLink(link);

                if (textTarget && shouldReplaceContactText(textTarget.textContent)) {
                    textTarget.textContent = emailText;
                }
            });

            document.querySelectorAll("[data-email-link], [data-config-href='contact.email']").forEach((element) => {
        ...
        function updateLogoLabels(brandName) {
            if (!brandName) return;

            document.querySelectorAll(".site-logo, .footer-logo, .mobile-menu__logo").forEach((link) => {
                const currentLabel = link.getAttribute("aria-label") || "";
                const nextLabel = currentLabel
                    ? currentLabel.replace(/Liftora/g, brandName)
                    : `${brandName} home`;

                link.setAttribute("aria-label", nextLabel);
            });

            document.querySelectorAll("img[data-logo], .site-logo img, .footer-logo img, .mobile-menu__logo img").forEach((image) => {
                image.setAttribute("alt", `${brandName} logo`);
            });
        }
        ...
        function updateContactFormEmail(email) {
            ...
            document.querySelectorAll("form").forEach((form) => {
                if (form.hasAttribute("data-recipient")) {
                    form.setAttribute("data-recipient", email);
                }
        ...
        function updateFooterYearAndCopyright(brandName) {
            if (!brandName) return;
            ...
            document.querySelectorAll("[data-config='footer.copyright']").forEach((element) => {
                if (!element.textContent.trim()) {
                    element.textContent = `© ${currentYear} ${brandName}. All rights reserved.`;
                }
            });
        }

        function getTextTargetInsideLink(link) {
            const spans = Array.from(link.querySelectorAll("span"));

            const textSpan = spans.find((span) => span.textContent.trim());
        ...
        function isHeaderPhoneIconButton(element) {
            return Boolean(
                element &&
                element.matches &&
                element.matches(".header-actions .btn-icon[data-config-href='contact.phone']")
            );
        }

        function ensurePhoneIconOnly(link) {
            if (!link) return;

            link.replaceChildren(createIcon("phone"));
        }

        function shouldReplaceContactText(text) {
            ...
            return (
                ...
                value.includes("@liftora") ||
                value.includes("Platform Address") ||
        ...
        function shouldSkipElement(element) {
            ...
            return skippedTags.includes(element.tagName);
        }
    ...
    (function () {
        document.addEventListener("DOMContentLoaded", initContactCounters);

        function initContactCounters() {
            const zone = document.querySelector("[data-count-zone]");
            if (!zone) return;

            const counters = Array.from(zone.querySelectorAll("[data-count-to]"));
